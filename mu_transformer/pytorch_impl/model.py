...
class RotaryEncoding(nn.Module):
    def __init__(self, hps: TransformerConfig) -> None:
        ...
        positions = torch.arange(self.hps.sequence_len, device=self.hps.device)
        channels = torch.arange(self.hps.d_head // 2, device=self.hps.device)
        ang_freqs = torch.pow(self.hps.rotary_base, -channels / (self.hps.d_head // 2))

        # expand to a shape broadcastable with q/k dims
        positions = torch.reshape(positions, [1, self.hps.sequence_len, 1, 1])
        ang_freqs = torch.reshape(ang_freqs, [1, 1, 1, self.d_head // 2])

        radians = positions * ang_freqs
        cos = torch.cos(radians).to(self.hps.dtype)
        sin = torch.sin(radians).to(self.hps.dtype)
...
class TransformerBlock(nn.Module):
    ...
    def _forward(self, x):  # , intermediates, layer_id):
        ...
        r1 = self.mha(self.norm1(x))
        x = x + r1
        r2 = self.mlp(self.norm2(x))
        x = x + r2
        return x
